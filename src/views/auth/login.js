...
import qs from "qs";
import {
  ACCESS_TOKEN, APPOINTMENTS_ROUTE, BASE_ROUTE_PATH,

  REFRESH_TOKEN, ROLE_ADMIN, ROLE_PATIENT,
...
const Login = () => {
  ...
  const loginHandler = async (e) => {
    ...
    if (data.username.trim() === "")
      return notifyMessage("Username cannot be empty", 0);
    ...
    userLogin(qs.stringify(obj)).then((res) => {
      console.log("LOGIN",res)
      if (res.access_token) {
        localStorage.setItem(ACCESS_TOKEN, res.access_token);
        localStorage.setItem(REFRESH_TOKEN, res.refresh_token);
        localStorage.setItem("USER_ROLE", ROLE_PATIENT);
        // localStorage.setItem(USER_ROLE, userRole);
        // window.location = `${BASE_ROUTE_PATH}${PICKUP_ORDER_ROUTE}`;

        window.location.href = APPOINTMENTS_ROUTE

        // notifyMessage("Successfully login", 1);
      } else {
        if (res.message) {
